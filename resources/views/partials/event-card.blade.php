...
@if($event !== null && $event->segment !== null)
    <a class="event-card card {{ isset($isSmall) && $isSmall === true ? 'small' : '' }}" href="{{ route('event', [
        ...
        <figure class="event-card__image-wrapper lazy-image">
            @if($event->medium_image)
                <img class="event-card__image" data-src="{{$event->medium_image}}" src="{{ $event->thumbnail }}" alt="{{ $event->name }}" loading="lazy">
            @else
                <img class="event-card__image" data-src="{{ $event->poster }}" src="{{ $event->thumbnail }}" alt="{{ $event->name }}" loading="lazy">
            @endif
